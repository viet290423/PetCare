...
class PetTipsResponse {
  ...
  factory PetTipsResponse.fromJson(Map<String, dynamic> json) {
    ...
    return PetTipsResponse(
      tip: tip,
      categories: _parseCategories(tip),
    );
  }
  ...
  static PetTipsCategories? _parseCategories(String tip) {
    try {
      ...
      final healthMatch = RegExp(r'\*\*Sức khỏe\*\*|Sức khỏe:', caseSensitive: false).firstMatch(tip);
      final nutritionMatch = RegExp(r'\*\*Dinh dưỡng\*\*|Dinh dưỡng:', caseSensitive: false).firstMatch(tip);
      final careMatch = RegExp(r'\*\*Chăm sóc\*\*|Chăm sóc:', caseSensitive: false).firstMatch(tip);
      ...
      if (healthMatch != null) {
        ...
        healthContent = healthContent.replaceFirst(RegExp(r'\*\*Sức khỏe\*\*|Sức khỏe:'), '').trim();
      }
      ...
      if (nutritionMatch != null) {
        ...
        nutritionContent = nutritionContent.replaceFirst(RegExp(r'\*\*Dinh dưỡng\*\*|Dinh dưỡng:'), '').trim();
      }
      ...
      if (careMatch != null) {
        ...
        careContent = careContent.replaceFirst(RegExp(r'\*\*Chăm sóc\*\*|Chăm sóc:'), '').trim();
      }
      ...
      final healthCards = healthContent.isNotEmpty ? _parseCardsFromContent(healthContent, 'health') : null;
      final nutritionCards = nutritionContent.isNotEmpty ? _parseCardsFromContent(nutritionContent, 'nutrition') : null;
      final careCards = careContent.isNotEmpty ? _parseCardsFromContent(careContent, 'care') : null;
  ...
  static List<PetTipCard> _parseCardsFromContent(String content, String category) {
    try {
      ...
      for (String line in lines) {
        ...
        // Kiểm tra xem có phải là số thứ tự không
          ...
          final cardContent = trimmedLine.replaceFirst(RegExp(r'^\d+\.\s*'), '').trim();
      ...
      if (cards.isEmpty && content.isNotEmpty) {
        cards.add(_createCardFromContent(
          _getDefaultTitle(category), 
          content, 
          category
        ));
      }
  ...
  static PetTipCard _createCardFromContent(String title, String description, String category) {
  ...
  static Map<String, String> _getIconAndColor(String category, String title) {
    ...
    // Health category
    if (category == 'health') {
      if (lowerTitle.contains('khám') || lowerTitle.contains('kiểm tra') || lowerTitle.contains('định kỳ')) {
        return {'icon': 'medical_services', 'color': 'blue'};
      } else if (lowerTitle.contains('phòng bệnh') || lowerTitle.contains('vaccine') || lowerTitle.contains('phòng ngừa')) {
        return {'icon': 'vaccines', 'color': 'green'};
      } else if (lowerTitle.contains('triệu chứng') || lowerTitle.contains('theo dõi') || lowerTitle.contains('quan sát')) {
        return {'icon': 'monitor_heart', 'color': 'red'};
      } else if (lowerTitle.contains('dị ứng') || lowerTitle.contains('allergy')) {
    ...
    if (category == 'nutrition') {
      if (lowerTitle.contains('thực phẩm') || lowerTitle.contains('nên dùng') || lowerTitle.contains('ăn')) {
        return {'icon': 'restaurant', 'color': 'orange'};
      } else if (lowerTitle.contains('nên tránh') || lowerTitle.contains('tránh') || lowerTitle.contains('không nên')) {
        return {'icon': 'no_food', 'color': 'red'};
      } else if (lowerTitle.contains('khẩu phần') || lowerTitle.contains('lượng') || lowerTitle.contains('định lượng')) {
    ...
    // Care category
    if (category == 'care') {
      if (lowerTitle.contains('vệ sinh') || lowerTitle.contains('tắm') || lowerTitle.contains('làm sạch')) {
        return {'icon': 'shower', 'color': 'blue'};
      } else if (lowerTitle.contains('môi trường') || lowerTitle.contains('nhà') || lowerTitle.contains('sống')) {
        return {'icon': 'home', 'color': 'brown'};
      } else if (lowerTitle.contains('vận động') || lowerTitle.contains('chơi') || lowerTitle.contains('tinh thần')) {
        return {'icon': 'fitness_center', 'color': 'green'};
      } else if (lowerTitle.contains('tinh thần') || lowerTitle.contains('stress') || lowerTitle.contains('tâm lý')) {
  ...
  factory PetTipsCategories.fromJson(Map<String, dynamic> json) {
    return PetTipsCategories(
      ...
      healthCards: json['healthCards'] != null 
          ? (json['healthCards'] as List).map((e) => PetTipCard.fromJson(e)).toList()
          : null,
      nutritionCards: json['nutritionCards'] != null 
          ? (json['nutritionCards'] as List).map((e) => PetTipCard.fromJson(e)).toList()
          : null,
      careCards: json['careCards'] != null 
          ? (json['careCards'] as List).map((e) => PetTipCard.fromJson(e)).toList()
          : null,
    );
  }

  bool get hasAnyContent => health != null || nutrition != null || care != null;
  bool get hasAnyCards => (healthCards?.isNotEmpty ?? false) || 
...
class PetGrowthResponse {
  ...
  final String? warningType; // 'sudden_increase', 'sudden_decrease', 'gradual_increase', 'gradual_decrease'
